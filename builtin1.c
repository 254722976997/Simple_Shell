 ...
 * Return: Always 0
 */
int displayHistory(info_t *info, list_t *list)
{
...
/**
 * unsetAlias - Unset alias for the given string.
...
int unsetAlias(info_t *info, char *str)
{
	...
	c = *p;
	*p = 0;
	ret = delete_node_at_index(&(info->alias), get_node_index(info->alias, find-node_starts_with(info->alias, str, -1)));
	*p = c;
...
/**
 * setAlias - Set alias to the given string.
 * @info: Parameter struct
 * @str: The string alias
 *
 * Return: Always 0 on success, 1 on error
